 ...
 p(ReadBlock);
         for(int i = 0; i < NOC; i++)
	 {
	   if(OB[i].Done == 0)
		{
		   sleep(1);
		   cout << *Ptime << " Waiter ID " << id << ": performs order of Customer ID: " << i << " (" << menu[OB[i].ItemId].Name << ", " << OB[i].Amount << ")\n";
		   OB[i].Done=1;
	   	   menu[OB[i].ItemId].TotalOrdered+=OB[i].Amount;
		}
	 }
  v(ReadBlock);
    ...
    *WaitersCount=0;
	...
	if(i<NumOfCustProc)//(*CustomerNumber)!=0)        
	{
		...
		p(CN);
			...
			(*CustomerNumber)--;
			cout <<" the Customer number is: "<<(*CustomerNumber)<<"\n";
		v(CN);
		while(*Ptime<15)
        	{
			sleep(randNum(3, 6));
	...
	else if(i>=NumOfCustProc && i<totalnum)//(*CustomerNumber==0) &&  (*WaitersNumber)!=0)        
	{
		
		p(WN);
			cout << *Ptime << " Waiter ID " << *WaitersCount << ": created PID " << getpid() << " PPID " << getppid() << "\n";
			(*WaitersNumber)--;
			(*WaitersCount)++;
		v(WN);
		while(*Ptime<15)
